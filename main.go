...
func (g *Game) checkForCollision(p1, p2 *planet.Planet) bool {
	...
	if math.Abs(float64(distVec.X)) <= radSum && math.Abs(float64(distVec.Y)) <= radSum && math.Abs(float64(distVec.Z)) <= radSum { // Box collision to do a rough check
...
func main() {
	...
	*/
	...
	g.makeCube(rl.NewVector3(0.0, 100.0, 0.0), w, h, d, 60, 3)
